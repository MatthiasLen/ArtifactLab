...
def _gaussian_bias_field(
    ...
    """Build a normalized Gaussian multiplicative bias field in image space."""

    height = shape[-2]
    width = shape[-1]
    ky = torch.linspace(-1.0, 1.0, steps=height, device=device)
    kx = torch.linspace(-1.0, 1.0, steps=width, device=device)
    ky_grid, kx_grid = torch.meshgrid(ky, kx, indexing="ij")

    dx = (kx_grid - center_x_fraction) / width_x_fraction
    dy = (ky_grid - center_y_fraction) / width_y_fraction
    gaussian = torch.exp(-0.5 * (dx * dx + dy * dy))
...
class GaussianBiasField(BaseDistortion):
    """Smooth centered multiplicative bias field in image space.
    ...
    :param float width_fraction: Radial width of the Gaussian envelope on the
        normalized k-space grid.
    :param float edge_gain: Gain approached near the edge of image space. Must lie
        in ``(0, 1]``.
    """
    ...
    def A(self, y: torch.Tensor) -> torch.Tensor:
        gain = self._gain_field(y.shape, y.device)
        B = 1.0 / (torch.exp(gain))
        return image_to_shifted_kspace(shifted_kspace_to_image(y) * B)
...
class OffCenterAnisotropicGaussianBiasField(BaseDistortion):
    """Off-center anisotropic Gaussian multiplicative bias field .



    :param float width_x_fraction:
    :param float width_y_fraction:
    :param float center_x_fraction:
