...
from mri_recon.reconstruction.deep import RAMReconstructor, DeepImagePriorReconstructor
from mri_recon.reconstruction.classic import ZeroFilledReconstructor, ConjugateGradientReconstructor, TVPGDReconstructor, WaveletFISTAReconstructor, TVFISTAReconstructor
...
def test_reconstructors(name, device):
    ...
    """
    x = dinv.utils.load_example("butterfly.png", img_size=(32, 32), grayscale=True, resize_mode="resize", device=device)
    x = torch.cat([x, torch.zeros_like(x)], dim=1) # dummy complex data

    model = choose_algorithm(name, img_size=x.shape[1:], device=device)
    
    physics = DistortedKspaceMultiCoilMRI(img_size=(1, 2, *x.shape[-2:]), coil_maps=1, device=device)
