...
@pytest.mark.parametrize("img_size", [(1, 2, 256, 256), (1, 2, 4, 256, 256)]) # singlecoil and multicoil
def test_distortion_properties(name, img_size, device):
    ...
    if len(img_size) == 4: # singlecoil
        ...
        coil_maps = torch.ones(1, *img_size[-3:], device=device, dtype=torch.complex64) / sqrt(img_size[-3])
    physics = DistortedKspaceMultiCoilMRI(distortion=distortion, img_size=(1, 2, *y.shape[-2:]), coil_maps=coil_maps, device=device)
